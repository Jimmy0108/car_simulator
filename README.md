...
數學方程式：$h(x, y, \theta) = \max(h_{Dijkstra}(x, y), h_{RS}(x, y, \theta))$

2. Voronoi 安全勢能場 (Voronoi Potential Field)

為解決路徑過於貼近障礙物的危險，系統在成本函數中加入了基於廣義 Voronoi 圖 (GVD) 的勢能評估。
系統會計算車身四個角落到障礙物的最短距離 $d_o$，當 $d_o$ 小於有效半徑 $d_o^{max}$ 時，施加排斥力：

勢能方程式：$\rho_v(x,y) = \left( \frac{\alpha}{\alpha + d_o} \right) \left( \frac{d_o - d_o^{max}}{d_o^{max}} \right)^2$

3. 多階段動態最佳化 (Multi-stage Dynamic Optimization)

針對狹窄車道難以直接入庫的物理極限，系統動態在目標車位外的通道上採樣允許集合 (Admissible Set $\Omega$)，並計算最佳中繼點 (Waypoint)，將複雜任務拆解為「入口 $\rightarrow$ 中繼點 $\rightarrow$ 車位」兩階段規劃。
