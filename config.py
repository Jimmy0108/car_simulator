...
PARAMS = {
    ...
    'w3': 25.5,  # 控制 Voronoi 安全場（防撞牆） 的權重
    'w_lane': 0.5,  # 車道中心偏移軟懲罰權重 (quadratic)
    'u1': 110.0,  # Voronoi場的潛在函數權重，用於最大化與障礙物的距離。
    'u2': 10.0,  # 路徑平滑的代價函數權重，與曲率有關。
    'u3': 5.0,   # 路徑平滑的代價函數權重，與曲率變化率有關。
    'u4': 10.0,  # 路徑平滑的代價函數權重，與急動度（jerk）有關。
    'alpha': 50.2,  # Voronoi場的潛在函數中的比例因子，用於調整場的強度。
    ...
    'rs_shot_interval': 8,  # 週期性 RS 解析展開觸發間隔（每展開 N 個節點嘗試一次）。
    'rs_shot_dist_threshold': 2.0,  # 與目標距離小於此值時，強制嘗試 RS 解析展開。
    ...
    'w_rho': 10.0,      # $w_\rho$ 數據保持項權重：維持參考路徑形狀，避免平滑過度偏離
    'w_o': 50.0,        # $w_o$ 障礙物/碰撞懲罰權重：中心與四角距離低於閾值時的二次懲罰
    'w_kappa': 0.5,     # $w_\kappa$ 曲率懲罰權重：限制瞬間曲率符合非完整約束
    'w_s': 0.5,         # $w_s$ 平滑度懲罰權重：最小化二階差分，使節點分佈更均勻
